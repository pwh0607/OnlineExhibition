...
public class FBImgLoader : MonoBehaviour
{
    ...
    {
        //변수값 초기화
        roomNumber = "Room1";           //나중에 scene이름으로 변경.

        //데이터베이스 참조
        FirebaseApp.DefaultInstance.Options.DatabaseUrl = new System.Uri("https://onlineexhibition-9dbef-default-rtdb.firebaseio.com/");
        databaseRef = FirebaseDatabase.DefaultInstance.RootReference.Child("rooms").Child(roomNumber);

        //스토리지 참조
        storage = FirebaseStorage.DefaultInstance;
        storageRef = storage.GetReferenceFromUrl("gs://onlineexhibition-9dbef.appspot.com/Room1");
    ...
    public void ReadDB(string _dataSet)
    {
        // 특정 데이터셋의 DB 참조 얻기
        databaseRef.GetValueAsync().ContinueWith(
             task =>
             {
                 ...
                 else if (task.IsCompleted)
                 {
                    ...
                    Debug.Log("photo 1 : " + dirInfo[gameObject.name]);
                    string photoPath = Convert.ToString(dirInfo[gameObject.name]);
                    GetImage(photoPath);
                 }
             }
         );
